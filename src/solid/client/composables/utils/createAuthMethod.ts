// src/solid/client/composables/createAuthMethod.ts

/**
 * Composable for dynamic authentication method selection
...
/**
 * Composable for managing authentication method selection
...
export function createAuthMethod() {
  ...
  const defaultMethod = createMemo(
    () => config().features?.defaultSignInMethod || 'password'
  )
